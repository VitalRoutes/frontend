...
function Modal({ isOpen, onClose, activateFunction, title, children }: PropsWithChildren<ModalProps>) {
    ...
    return isOpen ? (
        <div className="fixed top-0 left-0 flex items-center justify-center w-screen h-screen z-50">
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center xl:justify-start justify-center xl:w-[420px] w-[280px] xl:h-auto h-52 xl:p-14 p-6 bg-white rounded-xl">
                ...
                <button
                    className='xl:w-[210px] w-[140px] xl:h-11 h-10 absolute bottom-0 left-0 rounded-l-lg bg-gray-6 xl:text-xl text-base text-gray-1 font-bold leading-8 z-40'
                    type="button"
                    onClick={onClose}>취소</button>
                <button
                    className='xl:w-[210px] w-[140px] xl:h-11 h-10 absolute bottom-0 right-0 rounded-r-lg bg-green-1 xl:text-xl text-base text-gray-1 font-bold leading-8 z-40'
                    type="button"
                    onClick={activateFunction}>확인</button>
            </div>
        </div>
    ) : null
}
